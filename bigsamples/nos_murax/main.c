...
void main(void)
{
    ...
    err = nOS_ThreadCreate(&FooHandle9, Foo9, 0, FooStack9, FOO_STACK_SIZE, 1, NOS_THREAD_READY, "Foo9");
    err = nOS_ThreadCreate(&FooHandle3, Foo3, 0, FooStack3, FOO_STACK_SIZE, 1, NOS_THREAD_READY, "Foo3");
    if(err) printf("Err:%d\n",err);
    nOS_Start();   
    enable_interrupts();
