...
for name in src/*/; do
	...
	RESDIR=result/${name}
	mkdir ${RESDIR}
	${LCCDIR}/lcc -Wo-lccdir=${LCCDIR} -S -target=riscv32 ${SRCDIR}/${name}.c -o ${RESDIR}/${name}.s
	...
	${BINUTILSDIR}/as -c -o ${RESDIR}/${name}.o ${RESDIR}/${name}.s
    libs=$(<./src/${name}/libs.dep)
    objs=$(<./src/${name}/objs.dep)
	${BINUTILSDIR}/ld -c -h -o ${RESDIR}/${name}.bin ${libs} ${RESDIR}/start.o ${RESDIR}/io.o ${RESDIR}/${name}.o ${objs} 
